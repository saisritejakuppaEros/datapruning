...
### Integrated pipeline (CLIP embeddings + dataset)

To run the full pipeline from CLIP embeddings and dataset path (no SemDeDup modifications):

```bash
# Edit config at top of script, then run from repo root
bash codes/effective_pruning/run_effective_pruning.sh
```

**Prerequisites:**
- CLIP embeddings in parquet format at `output/clip_embeds/`
- Dataset at the path in `DATASET_PATH`
- SemDeDup conda env (faiss, etc.) and effective_pruning deps (qpsolvers, pygtrie)
