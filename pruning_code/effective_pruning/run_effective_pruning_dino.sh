...
DATASET_INFO="${EMBEDDINGS_DIR}/dataset_info.json"
TOTAL_SIZE=$(python3 -c "import json; print(json.load(open('${DATASET_INFO}'))['dataset_size'])")
EMB_SIZE=$(python3 -c "import json; print(json.load(open('${DATASET_INFO}'))['emb_size'])")
...
echo "=== Step 3: Compute d_inter (emb_dim=${EMB_SIZE}) ==="
