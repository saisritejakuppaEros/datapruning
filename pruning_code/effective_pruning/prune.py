...
import logging
from utils import seed_everything, build_and_save_trie, load_arrays, get_logger
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output .txt file for kept paths (SemDeDup format, one per line). Default: save_dir/kept_paths_effective_pruning.txt",
    )
    ...
    # Write .txt in SemDeDup format (one path per line)
    output_txt = args.output_txt
    if output_txt is None:
        output_txt = os.path.join(args.save_dir, "kept_paths_effective_pruning.txt")
    with open(output_txt, "w") as fp:
        fp.write("\n".join(all_paths_pruned))
    print_fn(f"Kept paths (txt) saved to {output_txt}")
