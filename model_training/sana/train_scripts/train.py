...
from diffusion.model.repa_utils import (
    build_repa_projectors,
    compute_proj_loss,
...
def train(
    ...
    skip_step = (global_step - 1) % train_dataloader_len + 1
    ...
    for epoch in range(start_epoch + 1, config.train.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            repa_active = repa_cfg is not None and repa_cfg.enabled and repa_encoders
            if repa_active:
                raw_images = (batch[0].to(accelerator.device) + 1) / 2 * 255.0
                with torch.no_grad():
                    pixel_imgs = preprocess_for_encoder(
                        raw_images,
                        enc_type=repa_cfg.enc_type,
                        resolution=getattr(repa_cfg, "resolution", image_size),
                    )
                    zs = extract_encoder_features(
                        repa_encoders, repa_encoder_types, pixel_imgs,
                        resolution=getattr(repa_cfg, "resolution", image_size),
                    )
                # DDP wraps model; use .module to access repa_projectors
                unwrapped = model.module if hasattr(model, "module") else model
                num_proj = len(unwrapped.repa_projectors) if hasattr(unwrapped, "repa_projectors") else 0
                zs_aligned = sum([[z] * num_proj for z in zs], [])
            ...
            with accelerator.accumulate(model):
                ...
                if repa_active:
                    model_kwargs["return_hidden"] = True
                loss_term = train_diffusion.training_losses(
                    model, clean_images, timesteps, model_kwargs=model_kwargs
                )
                diff_loss = loss_term["loss"].mean()
                if repa_active and "zs_tilde" in loss_term:
                    proj_loss = compute_proj_loss(zs_aligned, loss_term["zs_tilde"], device=diff_loss.device)
                    loss = diff_loss + repa_cfg.proj_coeff * proj_loss
                else:
                    loss = diff_loss
                    proj_loss = torch.tensor(0.0, device=diff_loss.device, dtype=diff_loss.dtype)
            ...
            if repa_active:
                proj_t = proj_loss if isinstance(proj_loss, torch.Tensor) else torch.tensor(proj_loss, device=loss.device, dtype=loss.dtype)
                diff_t = diff_loss if isinstance(diff_loss, torch.Tensor) else torch.tensor(diff_loss, device=loss.device, dtype=loss.dtype)
                logs["proj_loss"] = accelerator.gather(proj_t).mean().item()
                logs["diff_loss"] = accelerator.gather(diff_t).mean().item()
            if grad_norm is not None:
                gn = grad_norm if isinstance(grad_norm, torch.Tensor) else torch.tensor(grad_norm, device=loss.device, dtype=loss.dtype)
                logs.update(grad_norm=accelerator.gather(gn).mean().item())
            ...
            if (step + 1) % config.train.log_interval == 0 or (step + 1) == 1:
                ...
                current_step = (global_step - 1) % train_dataloader_len + 1
...
def main(cfg: SanaConfig) -> None:
    ...
    global repa_cfg, repa_encoders, repa_encoder_types
    ...
    # DDP kwargs: find_unused_parameters=True needed for REPA projectors
    ...
    # 4-3. REPA setup (before optimizer so projectors are included)
    repa_cfg = getattr(config, "repa", None)
    repa_encoders = []
    repa_encoder_types = []
    if repa_cfg is not None and repa_cfg.enabled:
        if config.data.load_vae_feat:
            raise ValueError(
                "REPA requires raw pixel images. Set data.load_vae_feat=false when repa.enabled=true."
            )
        repa_resolution = getattr(repa_cfg, "resolution", image_size)
        logger.info(f"[REPA] Loading encoder(s): {repa_cfg.enc_type}")
        repa_encoders, repa_encoder_types, _ = load_repa_encoders(
            repa_cfg.enc_type, accelerator.device, resolution=repa_resolution
        )
        for enc in repa_encoders:
            enc.eval()
            for p in enc.parameters():
                p.requires_grad_(False)
        encoder_feat_dim = repa_encoders[0].embed_dim
        num_proj = getattr(repa_cfg, "encoder_depth", 8)
    ...
    if config.model.resume_from is not None and config.model.resume_from["checkpoint"] is not None:
        ...
        if config.model.resume_from["checkpoint"] is not None:
            ...
            # Resolve real path in case of symlinks like latest.pth
            real_ckpt_path = osp.realpath(config.model.resume_from["checkpoint"])
            path = osp.basename(real_ckpt_path)

        try:
            parts = path.replace(".pth", "").split("_")
            if "step" in parts:
                step_idx = parts.index("step") + 1
                start_step = int(parts[step_idx])
                if train_dataloader_len > 0:
                    start_epoch = start_step // train_dataloader_len
                    logger.info(f"Calculated start_epoch {start_epoch} from step {start_step} and dataloader_len {train_dataloader_len}")
                else:
                    start_epoch = int(parts[1]) - 1
                    logger.warning(f"train_dataloader_len is 0! Falling back to epoch {start_epoch+1} from filename.")
            else:
                start_epoch = int(parts[1]) - 1
                logger.info(f"Stepless checkpoint detected. Setting start_epoch to {start_epoch} from filename.")
        except Exception as e:
            logger.warning(f"Failed to parse epoch/step from {path}: {e}")
