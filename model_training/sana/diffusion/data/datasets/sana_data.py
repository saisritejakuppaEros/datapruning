...
class SanaWebDataset(Dataset):
    ...
    def getdata(self, idx):
        ...
        # Fallback logic for caption keys
        if caption_type not in info:
            # Try potential keys in order
            for key in [self.default_prompt, "prompt", "enhanced_prompt", "caption"]:
                if key in info:
                    caption_type = key
                    break
        
        # caption_type = caption_type if caption_type in info else self.default_prompt
        txt_fea = "" if info.get(caption_type) is None else info.get(caption_type)
    ...
    def get_data_info(self, idx):
        try:
            ...
            elif "image_resolution" in info:
                # fine-t2i format: [width, height] or [height, width]? usually [w, h] in resolution lists but checking usually safe
                w, h = info["image_resolution"] 
            else:
