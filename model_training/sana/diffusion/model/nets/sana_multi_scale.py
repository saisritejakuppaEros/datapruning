...
class SanaMS(Sana):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, return_logvar=False, jvp=False, **kwargs):
        ...
        When return_hidden=True and repa_enabled, returns (x, zs_tilde) where zs_tilde
        is a list of projected hidden states from selected blocks.
        """
        return_hidden = kwargs.pop("return_hidden", False)
        repa_active = return_hidden and getattr(self, "repa_enabled", False) and len(getattr(self, "repa_projectors", [])) > 0

        if repa_active:
            num_proj = len(self.repa_projectors)
            depth = len(self.blocks)
            step = max(1, depth // num_proj)
            selected_indices = list(range(0, depth, step))[:num_proj]
            hidden_states = []
        ...
        if self.cfg_embedder and data_info is not None and "cfg_scale" in data_info:
        ...
        for i, block in enumerate(self.blocks):
            ...
            if repa_active and i in selected_indices:
                hidden_states.append(x)

        x = self.final_layer(x, t)  # (N, T, patch_size ** 2 * out_channels)
        x = self.unpatchify(x)  # (N, out_channels, H, W)

        if repa_active and hidden_states:
            zs_tilde = []
            for proj, h in zip(self.repa_projectors, hidden_states):
                zs_tilde.append([proj(h)])
            return x, zs_tilde
...
class SanaMSCM(SanaMS):
    def forward(self, x, timestep, y, data_info=None, return_logvar=False, **kwargs):
        ...
        if return_hidden and isinstance(super_out, tuple):
            model_out, zs_tilde = super_out
        elif return_logvar:
            model_out, logvar = super_out
        else:
            model_out = super_out
        ...
        if return_hidden and isinstance(super_out, tuple):
            return trigflow_model_out, zs_tilde
        elif return_logvar:
