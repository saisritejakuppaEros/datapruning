...
class GaussianDiffusion:
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if isinstance(model_output, tuple) and len(model_output) == 2 and isinstance(model_output[1], list):
                output, terms["zs_tilde"] = model_output
            elif isinstance(model_output, tuple) and len(model_output) == 2:
                output = model_output[0]
            elif isinstance(model_output, dict) and model_output.get("x", None) is not None:
